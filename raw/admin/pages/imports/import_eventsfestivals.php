<?php
// imports/import_eventsfestivals.php
...
// Define the exact fields from your template (EXCLUDING villageid and eventsfestivalsid)
$expected_headers = [
    'eventname',
    'eventtype',
    'startdate',
...
if (isset($_FILES['excel_file']) && $_FILES['excel_file']['error'] == 0) {
    ...
    if (!empty($imported_data)) {
        ...
        $village_res = $Event->selectdata("villagebasic", "select villageid from villagebasic");
        ...
        foreach ($imported_data as $row_data) {
            ...
            // Skip if no event name or event type (required fields based on your schema)
            if (empty($row_data['eventname']) || empty($row_data['eventtype'])) {
                $error_count++;
                continue;
            }

            // Check for duplicate event name + event type combination
            $checkDuplicate = "SELECT * FROM $table_name WHERE eventname='" . $Event->escape($row_data['eventname']) . "' AND eventtype='" . $Event->escape($row_data['eventtype']) . "'";
            $chk = $Event->selectdata($table_name, $checkDuplicate);

            if ($chk == "No Data Found!") {
                // Try to insert
                if (insertEventsFestivalRecord($row_data, $Event, $table_name)) {
                    $inserted_count++;
                } else {
                    $error_count++;
                }
            ...
            <div class='container results-container'>
                <div class='card'>
                    <div class='card-header'>
                        <h4 class='card-title'>Events & Festivals Import Results</h4>
                    </div>
                    <div class='card-body'>
        ...
        echo "<table class='table table-sm summary-table'>
                ...
                <tr><td><strong>Duplicates (Event+Type):</strong></td><td><span class='badge bg-warning'>$duplicate_count records</span></td></tr>
                <tr><td><strong>Errors:</strong></td><td><span class='badge bg-danger'>$error_count records</span></td></tr>
              </table>";
        echo "</div>
                        <div class='d-flex gap-2 flex-wrap'>
                            <a href='../editform.php?tablename=eventsfestivals' class='btn btn-primary back-btn'>🎉 View Events List</a>
                            <a href='../eventsfestivals.php' class='btn btn-secondary back-btn'>➕ Add New Event</a>
                            <a href='../eventsfestivals.php#import-section' class='btn btn-info back-btn'>📥 Import More</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <body>
            <div class='container results-container'>
                <div class='alert alert-warning'>
                    ...
                    <p>Please use the downloaded template and ensure you have data in the required fields (Event Name, Event Type).</p>
                </div>
                <a href='../eventsfestivals.php' class='btn btn-primary'>🔄 Try Again</a>
            </div>
        </body>
        </html>";
    }
    ...
    if (preg_match_all('/<tr[^>]*>(.*?)<\/tr>/s', $html_content, $tr_matches)) {
        foreach ($tr_matches[1] as $index => $tr_content) {
            ...
            if (preg_match_all('/<(?:td|th)[^>]*>(.*?)<\/(?:td|th)>/s', $tr_content, $td_matches)) {
                ...
                // Only add row if we have meaningful data (at least eventname or eventtype)
...
// Function to insert events festival record into database (villageid auto-added)
function insertEventsFestivalRecord($data, $Event, $table_name)
{
    try {
                    ...
                    case 'date':
                        ...
                        if (!empty($value)) {
                            ...
                            if ($date && $date->format('Y-m-d') === $value) {
                                $clean_data[$field] = $Event->escape($value);
                            } else {
                        ...
                        break;
                    case 'string':
                        $clean_data[$field] = !empty($value) ? $Event->escape(trim($value)) : '';
                        break;
                    case 'text':
                        $clean_data[$field] = !empty($value) ? $Event->escape(trim($value)) : '';
                        break;
        ...
        // Validate event type (from your dropdown options)
        $valid_types = [
            'Cultural',
            'Music',
            'Film',
        ...
        if (!empty($clean_data['eventtype']) && !in_array($clean_data['eventtype'], $valid_types)) {
            $clean_data['eventtype'] = 'Other'; // Default to Other
        } elseif (empty($clean_data['eventtype'])) {
            $clean_data['eventtype'] = 'Other'; // Default if empty
        }

        // Validate contact number (10-digit phone)
        if (!empty($clean_data['contactnumber']) && !preg_match('/^\d{10}$/', $clean_data['contactnumber'])) {
            $clean_data['contactnumber'] = ''; // Clear invalid phone
        }

        // Validate date range (start date should be before or equal to end date)
        if (!empty($clean_data['startdate']) && !empty($clean_data['enddate'])) {
            $start_date = DateTime::createFromFormat('Y-m-d', $clean_data['startdate']);
            $end_date = DateTime::createFromFormat('Y-m-d', $clean_data['enddate']);
            if ($start_date && $end_date && $start_date > $end_date) {
                // Swap dates if start is after end
                $temp_date = $clean_data['startdate'];
                $clean_data['startdate'] = $clean_data['enddate'];
                $clean_data['enddate'] = $temp_date;
            }
        }
        ...
        $values = [
            $clean_data['eventname'],
            $clean_data['eventtype'],
            $clean_data['startdate'],
            $clean_data['enddate'],
        ...
        $result = $Event->insertdata($table_name, $query);

        return ($result == "Data Inserted.");
    } catch (Exception $e) {
        error_log("Events festivals import error: " . $e->getMessage());
        return false;
    }
}
