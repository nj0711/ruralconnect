<?php
// imports/import_restaurants.php
...
// Define the exact fields from your template (EXCLUDING villageid and restaurantsid)
$expected_headers = [
    'restaurantname',
    'photo',
    'address',
...
if (isset($_FILES['excel_file']) && $_FILES['excel_file']['error'] == 0) {
    ...
    if (!empty($imported_data)) {
        ...
        foreach ($imported_data as $row_data) {
            ...
            // Skip if no restaurant name (required field based on your schema)
            if (empty($row_data['restaurantname'])) {
                $error_count++;
                continue;
            }

            // Check for duplicate restaurant name
            $checkDuplicate = "SELECT * FROM $table_name WHERE restaurantname='" . $obj->escape($row_data['restaurantname']) . "'";
            $chk = $obj->selectdata($table_name, $checkDuplicate);

            if ($chk == "No Data Found!") {
                // Try to insert
                if (insertRestaurantRecord($row_data, $obj, $table_name)) {
                    $inserted_count++;
                } else {
                    $error_count++;
                }
            ...
            <div class='container results-container'>
                <div class='card'>
                    <div class='card-header'>
                        <h4 class='card-title'>Restaurants Import Results</h4>
                    </div>
                    <div class='card-body'>
        ...
        echo "<table class='table table-sm summary-table'>
                ...
                <tr><td><strong>Duplicates (Restaurant Name):</strong></td><td><span class='badge bg-warning'>$duplicate_count records</span></td></tr>
                <tr><td><strong>Errors:</strong></td><td><span class='badge bg-danger'>$error_count records</span></td></tr>
              </table>";
        echo "</div>
                        <div class='d-flex gap-2 flex-wrap'>
                            <a href='../editform.php?tablename=restaurants' class='btn btn-primary back-btn'>🍽️ View Restaurants List</a>
                            <a href='../restaurants.php' class='btn btn-secondary back-btn'>➕ Add New Restaurant</a>
                            <a href='../restaurants.php#import-section' class='btn btn-info back-btn'>📥 Import More</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <body>
            <div class='container results-container'>
                <div class='alert alert-warning'>
                    ...
                    <p>Please use the downloaded template and ensure you have data in the required fields (Restaurant Name).</p>
                </div>
                <a href='../restaurants.php' class='btn btn-primary'>🔄 Try Again</a>
            </div>
        </body>
        </html>";
    }
    ...
    if (preg_match_all('/<tr[^>]*>(.*?)<\/tr>/s', $html_content, $tr_matches)) {
        foreach ($tr_matches[1] as $index => $tr_content) {
            ...
            if (preg_match_all('/<(?:td|th)[^>]*>(.*?)<\/(?:td|th)>/s', $tr_content, $td_matches)) {
                ...
                // Only add row if we have meaningful data (at least restaurantname)
...
// Function to insert restaurant record into database (villageid auto-added, photo as empty JSON)
function insertRestaurantRecord($data, $obj, $table_name)
{
    try {
            ...
            if (isset($field_types[$field])) {
                switch ($field_types[$field]) {
                    ...
                    case 'text':
                        ...
                        break;
                    case 'json':
                        // Handle timeschedule as JSON (open/close times)
        ...
        // Validate booking process (online, offline, both)
        $valid_booking = ['online', 'offline', 'both'];
        if (!empty($clean_data['bookingprocess']) && !in_array($clean_data['bookingprocess'], $valid_booking)) {
            $clean_data['bookingprocess'] = 'both'; // Default to both
        } elseif (empty($clean_data['bookingprocess'])) {
            $clean_data['bookingprocess'] = 'both'; // Default if empty
        }

        // Validate contact number (10-digit phone)
        if (!empty($clean_data['contactno']) && !preg_match('/^\d{10}$/', $clean_data['contactno'])) {
            $clean_data['contactno'] = 0; // Clear invalid phone (bigint field)
        }
        ...
        // Validate website link (basic URL validation)
        if (!empty($clean_data['websitelink'])) {
            $url_pattern = '/^https?:\/\/[a-zA-Z0-9\-\.]+\.[a-zA-Z\.]{2,6}([\/\w \-.]*)*\/?$/';
            if (!preg_match($url_pattern, $clean_data['websitelink'])) {
                $clean_data['websitelink'] = ''; // Clear invalid URL
            }
        }
        ...
        $values = [
            ...
            $clean_data['restaurantname'],
            $clean_data['full_address'], // Combined address@city@zip
            $clean_data['contactno'],
            $clean_data['timeschedule'],
            $clean_data['cuisineserved'],
            $clean_data['bookingprocess'],
            $clean_data['websitelink'],
            $clean_data['customerreviews'],
        ...
        error_log("Restaurants import error: " . $e->getMessage());
        return false;
    }
}
