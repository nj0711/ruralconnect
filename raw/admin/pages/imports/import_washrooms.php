...
// Define the exact fields from your template (EXCLUDING villageid and washroomsid)
$expected_headers = [
    'numberofwashrooms',
    'locationdescription',
    'facilitytype',
...
if (isset($_FILES['excel_file']) && $_FILES['excel_file']['error'] == 0) {
    ...
    if (!empty($imported_data)) {
        ...
        foreach ($imported_data as $row_data) {
            ...
            if (empty($row_data['numberofwashrooms']) || empty($row_data['locationdescription']) || empty($row_data['entitytype'])) {
                $error_count++;
                continue;
            }

            // Check for duplicate location description
            $checkLocation = "SELECT * FROM $table_name WHERE locationdescription='" . $obj->escape($row_data['locationdescription']) . "'";
            $chk = $obj->selectdata($table_name, $checkLocation);

            if ($chk == "No Data Found!") {
                // Try to insert
                if (insertWashroomsRecord($row_data, $obj, $table_name)) {
                    $inserted_count++;
                } else {
                    $error_count++;
                }
        ...
        echo "<table class='table table-sm summary-table'>
                ...
                <tr><td><strong>Duplicates (Location Description):</strong></td><td><span class='badge bg-warning'>$duplicate_count records</span></td></tr>
                <tr><td><strong>Errors:</strong></td><td><span class='badge bg-danger'>$error_count records</span></td></tr>
              </table>";
        echo "</div>
                        <div class='d-flex gap-2'>
                            <a href='../editform.php?tablename=washrooms' class='btn btn-primary back-btn'>👁️ View Washrooms List</a>
                            <a href='../washrooms.php' class='btn btn-secondary back-btn'>➕ Add New Washroom</a>
                            <a href='../washrooms.php' class='btn btn-info back-btn'>📥 Import More</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <body>
            <div class='container results-container'>
                <div class='alert alert-warning'>
                    ...
                    <p>Please use the downloaded template and ensure you have data in the required fields (Number of Washrooms, Location Description, Entity Type).</p>
                </div>
                <a href='../washrooms.php' class='btn btn-primary'>🔄 Try Again</a>
            </div>
        </body>
        </html>";
    }
...
// Function to parse HTML table format (matches your template - 13 columns)
    ...
    if (preg_match_all('/<tr[^>]*>(.*?)<\/tr>/s', $html_content, $tr_matches)) {
        foreach ($tr_matches[1] as $index => $tr_content) {
            ...
            if (preg_match_all('/<(?:td|th)[^>]*>(.*?)<\/(?:td|th)>/s', $tr_content, $td_matches)) {
                ...
                // Only add row if we have meaningful data (at least numberofwashrooms or locationdescription)
...
// Function to parse actual XLSX file (13 columns)
...
function insertWashroomsRecord($data, $obj, $table_name)
{
    try {
        // Define field mappings and data types
        $field_types = [
            'villageid' => 'integer',
            'numberofwashrooms' => 'integer',
            'locationdescription' => 'string',
            'facilitytype' => 'string',
            'washroomcondition' => 'string',
        ...
        $values = [
            ...
            $clean_data['numberofwashrooms'],
            $clean_data['locationdescription'],
            $clean_data['facilitytype'],
            $clean_data['washroomcondition'],
            $clean_data['maintenanceschedule'],
        ...
        error_log("Washrooms import error: " . $e->getMessage());
        return false;
    }
}
