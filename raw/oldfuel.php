        ...
        <section class="department-one-section">
            <div class="container">
                ...
                <div class="row row-gutter-30">
                    <?php
						if (empty($result)) {
							echo "<h2 style='margin-bottom: 50px;'>Fuel Station</h2>";
						}
						else {
						foreach ($result as $row => $station) { 
                            if($station['visibility']=='on'){
							$modalId = "myModal" . $row;
							?>
                    <!-- card -->
                            ...
                            <b><span class="close" onclick="closeModal('<?php echo $modalId; ?>')">&times;</span>
                                <div class="modal-body">
                                    <div class="text-content">
                                        <h1><?php echo $station['fuelstationname']; ?></h1>
                                        <br>
                                        <p><i class="fa-solid fa-location-dot">&nbsp;</i>
                                            <?php // Retrieve concatenated address, city, pincode
												$addressCityPincode = $station['address'];
												$modifiedAddress = str_replace('@', ', ', $addressCityPincode);
												echo $modifiedAddress ?></p>
                                        <p><strong>Types of fuels available</strong>:<br>
                                            <?php echo $station['typesoffuelavailable']; ?></p>
                                        <p><strong>Description</strong>: <br> <?php echo $station['description']; ?></p>
                                        <p><strong>Hours</strong>: (Hours might differ)<br>
                                            <?php $timings = json_decode($station['pumpstimeduration'], true); ?>
                                            Opening Time:<?php echo $timings['open']; ?> <br>
                                            Closing Time: <?php echo $timings['close']; ?>
                                        </p>
                                        <p><strong>Contact No</strong>:<?php echo $station['contactno']; ?></p>
                                    </div>
