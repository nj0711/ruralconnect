<?php
// imports/import_entertainment.php
...
// Define the exact fields from your template (EXCLUDING villageid and entertainmentid)
$expected_headers = [
    'name',
    'type',
    'address',
...
$createTableQuery = "
  ...
  `entertainmentid` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
if (!$obj->tableExists('entertainment')) {
...
if (isset($_FILES['excel_file']) && $_FILES['excel_file']['error'] == 0) {
    ...
    if (!empty($imported_data)) {
        ...
        foreach ($imported_data as $row_data) {
            ...
            // Check for duplicate name (since no unique field specified)
            ...
            if ($chk == "No Data Found!") {
                // Try to insert
                if (insertEntertainmentRecord($row_data, $obj, $table_name)) {
                    $inserted_count++;
                } else {
                    $error_count++;
                }
            ...
            <div class='container results-container'>
                <div class='card'>
                    <div class='card-header'>
                        <h4 class='card-title'>Entertainment Import Results</h4>
                    </div>
                    <div class='card-body'>
        ...
        echo "</div>
                        <div class='d-flex gap-2 flex-wrap'>
                            <a href='../editform.php?tablename=entertainment' class='btn btn-primary back-btn'>🎬 View Entertainment List</a>
                            <a href='../entertainment.php' class='btn btn-secondary back-btn'>🎪 Add New Venue</a>
                            <a href='../entertainment.php' class='btn btn-info back-btn'>📥 Import More</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <body>
            <div class='container results-container'>
                ...
                <a href='../entertainment.php' class='btn btn-primary'>🔄 Try Again</a>
            </div>
        </body>
        </html>";
    }
...
// Function to insert entertainment record into database (villageid auto-added, photo as empty JSON)
function insertEntertainmentRecord($data, $obj, $table_name)
{
    try {
            ...
            if (isset($field_types[$field])) {
                switch ($field_types[$field]) {
                    case 'integer':
                        ...
                        break;
                    case 'bigint':
                        $clean_data[$field] = !empty($value) ? (int)$value : 0; // Contact number as integer
                        break;
                    case 'string':
                    ...
                    case 'text':
                        ...
                        break;
                    case 'json':
                        // Handle timeschedule as JSON (weekdayopenkey/weekdayclosekey)
        ...
        // Validate type field (theater, funpark, other)
        $valid_types = ['theater', 'funpark', 'other'];
        if (!empty($clean_data['type']) && !in_array($clean_data['type'], $valid_types)) {
            $clean_data['type'] = 'theater'; // Default to theater
        } elseif (empty($clean_data['type'])) {
            $clean_data['type'] = 'theater'; // Default if empty
        }

        // Validate contact number (10-digit phone)
        if (!empty($clean_data['contactno']) && !preg_match('/^\d{10}$/', $clean_data['contactno'])) {
            $clean_data['contactno'] = 0; // Clear invalid phone
        }
        ...
        error_log("Entertainment import error: " . $e->getMessage());
        return false;
    }
}
