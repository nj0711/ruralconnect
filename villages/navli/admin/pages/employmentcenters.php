...
<head>
    ...
    <!-- Page Title -->
    <title> Employeement Centers | Admin Panel</title>
...
<body>
        ...
        ***********************************-->
        <div class="content-body">
            ...
            <div class="container-fluid">
                ...
                $village_id = $Emp->selectdata("villagebasic", "select villageid from villagebasic");
                ...
                ?>
                    ...
                    <script>
                        ...
                        if (confirm("Are you sure you want to proceed?")) {
                            ...
                            window.location.href = "editform.php?tablename=employmentcenters";
                        }
                    </script>
                ...
                if (isset($_GET['confirmeddeleteid'])) {
                    ...
                    $del = "DELETE FROM employmentcenters WHERE employmentcentersid=" . $deleteId;
                    $result = $Emp->deletedata("employmentcenters", $del);
                ...
                if ($fetchID != "") {
                    $fetchquery = "select * from employmentcenters where employmentcentersid=" . $fetchID;
                    $FormData = $Emp->selectdata("employmentcenters", $fetchquery);
                }
                $Vid = $villageid;
                $ServiceID = isset($FormData[0]['employmentcentersid']) ? $FormData[0]['employmentcentersid'] : "";
                $name = isset($FormData[0]['centername']) ? $FormData[0]['centername'] : "";
  ...
  `visibility` varchar(5) NOT NULL DEFAULT 'off'
                    ...
                    $employmentcentersID = isset($_POST['employmentcentersid']) ? $_POST['employmentcentersid'] : "";
                    $VillageID = $villageid;
                    $ServiceName = isset($_POST['ServiceName']) ? $Emp->escape($_POST['ServiceName']) : "";
                    $ServiceType = isset($_POST['ServiceType']) ? $Emp->escape($_POST['ServiceType']) : "";
                    $contactNo = isset($_POST['ContactNo']) ? $Emp->escape($_POST['ContactNo']) : "";
                    $Address = isset($_POST['Address']) ? $Emp->escape($_POST['Address']) : "";
                    $city = isset($_POST['city']) ? $Emp->escape($_POST['city']) : "";
                    ...
                    if (isset($_POST['employmentcentersid']) && $_POST['employmentcentersid'] != "") {
                        if ($ServiceName != "" && $ServiceType != "" && $contactNo != "" && $VillageID != "") {
                            $sql = "update employmentcenters set centername='$ServiceName', servicetype='$ServiceType', contactnumber='$contactNo', address='$Address_join', villageid=$VillageID where employmentcentersid=$employmentcentersID";
                            $result = $Emp->updatedata("employmentcenters", $sql);
                        ...
                        if ($ServiceName != "" && $ServiceType != "" && $contactNo != "" && $VillageID != "") {
                            ...
                            // echo '<div class="alert alert-warning" role="alert">Please Enter Complete Details! you Enter Incomplete Details! </div>'; #Normal button 
                            // echo '<div class="alert alert-warning alert-dismissible fade show" role="alert">
                            //     Please Enter Complete Details! You Entered Details Are Incomplete!!
                            //     <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                            //     </div>';
                    ...
                    <div class="col-xl-12 col-xxl-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Village Emploment Centers</h4>
                            </div>
                            <div class="card-body">
                                <div class="basic-form">
                                    <form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST">
                                        ...
                                        <div class="row">
                                            <input type="hidden" class="form-control" name="employmentcentersid"
                                                value="<?php echo $ServiceID; ?>">
                                            <div class="mb-3 col-md-6">
                                                <label class="form-label">Emploment-Center Name</label>
                                                <input type="text" class="form-control" name="ServiceName"
                                                    onkeypress="return (event.charCode >= 65 && event.charCode <= 90) || (event.charCode >= 97 && event.charCode <= 122) || event.charCode == 32"
                                                    value="<?php echo $name; ?>">
                                            </div>
                                            <div class="mb-3 col-md-6">
                                                <label class="text-label form-label">Emploment-Service Type</label>
                                                <select class="default-select form-control wide mb-3"
                                                    name="ServiceType">
                                                    <option value="Job Placement"
                                                        <?php if ($type == 'Job Placement') echo 'selected'; ?>>Job
                                                        Placement</option>
                                                    <option value="Skill Training"
                                                        <?php if ($type == 'Skill Training') echo 'selected'; ?>>Skill
                                                        Training</option>
                                                    <option value="Career Counseling"
                                                        <?php if ($type == 'Career Counseling') echo 'selected'; ?>>
