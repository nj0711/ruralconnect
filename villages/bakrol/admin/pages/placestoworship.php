...
?>
    ...
    <script>
        ...
        if (confirm("Are you sure you want to proceed?")) {
            ...
            window.location.href = "editform.php?tablename=placestoworship";
        }
    </script>
...
if (isset($_GET['confirmeddeleteid'])) {
    ...
    $sel = "select * from placestoworship where placestoworshipid=" . $deleteId;
    $res = $obj->selectdata("placestoworship", $sel);
    ...
    $del = "DELETE FROM placestoworship WHERE placestoworshipid=" . $deleteId;
    $result = $obj->deletedata("placestoworship", $del);
...
if (isset($_POST['insert'])) {
  ...
  `placestoworshipid` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  `visibility` varchar(5) NOT NULL DEFAULT 'off'
    ...
    if (!$obj->tableExists('placestoworship')) {
    ...
    if (isset($_FILES['photo']) && !empty($_FILES['photo']['name'][0])) {
        ...
        for ($i = 0; $i < $fileCount; $i++) {
            ...
            if ($fileError === UPLOAD_ERR_OK) {
                ...
                $newFileName = time() . $name . 'placestoworship' . $i . '.' . $fileExtension;
    ...
    $query = "INSERT INTO placestoworship (
		villageid, name, history, photo, address, timeschedule, contactno, description
	) VALUES (
		'$village_id', '$name', '$history', '$filesJson', '$fullAddress', '$time_duration', '$contact_no', '$description'
	)";

    $result = $obj->insertdata("placestoworship", $query);
...
if (isset($_GET['updateid'])) {
    $selq = "select * from placestoworship where placestoworshipid = " . $_GET['updateid'];
    $res = $obj->selectdata("placestoworship", $selq);
    // print_r($res);
    ...
    if ($res != null) {
        ...
        header('Location: placestoworship.php?updateid=' . $_GET['updateid']);
    }
}
...
if (isset($_POST['update'])) {
    ...
    $selQ = "select photo from placestoworship  where placestoworshipid  = " . $_GET['updateid'];
    $res = $obj->selectdata("placestoworship", $selQ);
    $p = $res[0]['photo'];
    $array = json_decode($p);
    $count = count($array);
    ...
    if (isset($_FILES['photo']) && !empty($_FILES['photo']['name'][0])) {
        ...
        for ($i = 0; $i < $fileCount; $i++) {
            ...
            if ($fileError === UPLOAD_ERR_OK) {
                $fileExtension = pathinfo($fileName, PATHINFO_EXTENSION);
                $newFileName = time() . $name . 'placestoworship' . $count + $i . '.' . $fileExtension;
                $destination = $uploadDir . $newFileName;
                if (in_array($fileExtension, $allowedExtensions) && $fileSize <= $maxFileSize) {
                    if (move_uploaded_file($fileTmpName, $destination)) {
                        // Store the filename in the array
    ...
    $updateq = "update placestoworship set name = '{$name}', history = '{$history}', photo = '{$filesJson}', address = '{$fullAddress}', timeschedule = '{$time_duration}', contactno = '{$contact_no}', description = '{$description}' where placestoworshipId ={$_GET['updateid']}";
    $result = $obj->updatedata("placestoworship", $updateq);
...
<head>
    ...
    <!-- Page Title -->
    <title> Places To Worship | Admin Panel</title>
        ...
        <div class="content-body">
            ...
            <div class="container-fluid">
                    ...
                    <div class="col-xl-12 col-xxl-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Places to Worship</h4>
                            </div>
                            <div class="card-body">
                                            ...
                                            <div class="col-lg-12  col-xxl-12">
                                                ...
                                                <div class="tab-content">
                                                    <div class="row">
                                                        ...
                                                        <div class=" mb-2">
                                                            <div class="mb-3">
                                                                <label class="text-label form-label">Historical
                                                                    Information*</label>
                                                                <textarea name="history"
                                                                    onkeypress="return (event.charCode >= 65 && event.charCode <= 90) || (event.charCode >= 97 && event.charCode <= 122) || event.charCode == 32"
                                                                    class="form-control"><?php echo $history ?>
                                                                </textarea>
                                                            </div>
                                                        </div>
                                                        <div class="mb-3">
                                                        ...
                                                        <div class="col-lg-6 mb-2">
                                                            ...
                                                            <div class="mb-3">
                                                                ...
                                                                <textarea
                                                                    onkeypress="return (event.charCode >= 65 && event.charCode <= 90) || (event.charCode >= 97 && event.charCode <= 122) || event.charCode == 32"
                                                                    name="description" class="form-control"> <?php echo $description; ?>
                                                                </textarea>
                                                            </div>
                ...
                <div class="import-section" style="margin: 30px 0; padding: 20px; border: 1px solid #ddd; border-radius: 8px; background-color: #f8f9fa;" id="import-section">
                    <h4>⛪ Bulk Import Places to Worship</h4>
                    <p class="text-muted mb-3">
                        <strong>How it works:</strong> Download the template, fill in the worship place data, and import.
                    ...
                    <div class="row align-items-center g-3">
                        <div class="col-md-4">
                            <a href="templates/placestoworship_template.php" class="btn btn-info w-100">
                                📥 Download Template
                            </a>
                        </div>
                        <div class="col-md-8">
                            <form action="imports/import_placestoworship.php" method="post" enctype="multipart/form-data" class="d-flex gap-2">
                    ...
                    <div class="mt-3 p-2 border rounded">
                        <small class="text-muted">
                            ...
                            <strong>📝 Optional fields:</strong> History, City, Zip, Contact, Description<br>
                            ...
                            • <strong>Time Schedule:</strong> JSON format <code>{"weekdayopenkey":"05:00","weekdayclosekey":"21:00","weekendopenkey":"04:00","weekendclosekey":"22:00"}</code><br>
                            • <strong>Address Format:</strong> Full address in Address column, City and Zip in separate columns<br>
                            • <strong>History:</strong> Brief historical information about the place<br>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.delete-btn').forEach(function(button) {
                button.addEventListener('click', function() {
                    ...
                    const tableName = "placestoworship";
