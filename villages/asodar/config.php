...
class ConnDb
{
    ...
    public function __construct($db)
    {
        ...
        if (!$this->conn) {
            ...
            if ($this->databaseExists($db)) {
                $this->mysqli = new mysqli($this->server, $this->user, $this->user_pass, $db);
            } else {
    ...
    public function databaseExists($db)
    {
        ...
        if ($res) {
            if ($res->num_rows == 1) {
                ...
                array_push($this->result, $db . "  does not exists!");
                return false;
            }
        }
        ...
        if ($this->tableExists($table)) {
            ...
            if ($res = $this->mysqli->query($sql)) {
                if ($res->num_rows > 0) {
                    ...
                    array_push($this->result, "Data Failed!");
                    print_r($this->result[0]);
                }
            }
        } else {
    ...
    public function tableExists($table)
    {
        ...
        if ($res->num_rows == 1) {
            ...
            array_push($this->result, $table . "  does not exists in DB!");
            return false;
        }
    }
