...
// Define the exact fields from your template (EXCLUDING villageid and hospitalsid)
$expected_headers = [
    'type',
    'name',
    'photo',
...
if (isset($_FILES['excel_file']) && $_FILES['excel_file']['error'] == 0) {
    ...
    if (!empty($imported_data)) {
        ...
        foreach ($imported_data as $row_data) {
            ...
            // Skip if no name or type (required fields based on your schema)
            if (empty($row_data['name']) || empty($row_data['type'])) {
                $error_count++;
                continue;
            }
            ...
            if ($chk == "No Data Found!") {
                // Try to insert
                if (insertHospitalRecord($row_data, $obj, $table_name)) {
                    $inserted_count++;
                } else {
                    $error_count++;
                }
            ...
            <div class='container results-container'>
                <div class='card'>
                    <div class='card-header'>
                        <h4 class='card-title'>Hospitals Import Results</h4>
                    </div>
                    <div class='card-body'>
        ...
        echo "</div>
                        <div class='d-flex gap-2 flex-wrap'>
                            <a href='../editform.php?tablename=hospitals' class='btn btn-primary back-btn'>🏥 View Hospitals List</a>
                            <a href='../hospitals.php' class='btn btn-secondary back-btn'>➕ Add New Hospital</a>
                            <a href='../hospitals.php#import-section' class='btn btn-info back-btn'>📥 Import More</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <body>
            <div class='container results-container'>
                <div class='alert alert-warning'>
                    ...
                    <p>Please use the downloaded template and ensure you have data in the required fields (Name, Type).</p>
                </div>
                <a href='../hospitals.php' class='btn btn-primary'>🔄 Try Again</a>
            </div>
        </body>
        </html>";
    }
    ...
    if (preg_match_all('/<tr[^>]*>(.*?)<\/tr>/s', $html_content, $tr_matches)) {
        foreach ($tr_matches[1] as $index => $tr_content) {
            ...
            if (preg_match_all('/<(?:td|th)[^>]*>(.*?)<\/(?:td|th)>/s', $tr_content, $td_matches)) {
                ...
                // Only add row if we have meaningful data (at least name or type)
...
// Function to insert hospital record into database (villageid auto-added, photo as empty JSON)
function insertHospitalRecord($data, $obj, $table_name)
{
    try {
            ...
            if (isset($field_types[$field])) {
                switch ($field_types[$field]) {
                    ...
                    case 'text':
                        ...
                        break;
                    case 'json':
                        // Handle timeduration as JSON (open/close times)
        ...
        // Validate type field (Hospital, Medical Shop, Care Center)
        $valid_types = ['Hospital', 'Medical Shop', 'Care Center'];
        if (!empty($clean_data['type']) && !in_array($clean_data['type'], $valid_types)) {
            $clean_data['type'] = 'Hospital'; // Default to Hospital
        } elseif (empty($clean_data['type'])) {
            $clean_data['type'] = 'Hospital'; // Default if empty
        }
        ...
        // Validate patient capacity (non-negative integer)
        if (!empty($clean_data['patientcapacity']) && $clean_data['patientcapacity'] < 0) {
            $clean_data['patientcapacity'] = 0; // Default to 0 for non-hospital types
        }
        ...
        error_log("Hospitals import error: " . $e->getMessage());
        return false;
    }
}
