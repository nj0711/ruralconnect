<?php
// imports/import_electrification.php
...
// Define the exact fields from your template (EXCLUDING villageid and electrificationid)
$expected_headers = [
    'companyname',
    'emergencycontactno',
    'energyresourcessolar',
    'energyresourceswind',
    'energyresourcescoal',
    'energyresourcesgas',
    'photo',
...
$createTableQuery = "
  ...
  `electrificationid` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
if (!$obj->tableExists('electrification')) {
...
if (isset($_FILES['excel_file']) && $_FILES['excel_file']['error'] == 0) {
    ...
    if (!empty($imported_data)) {
        ...
        foreach ($imported_data as $row_data) {
            ...
            // Skip if no company name or office address (required fields based on your schema)
            if (empty($row_data['companyname']) || empty($row_data['officeaddress'])) {
                $error_count++;
                continue;
            }

            // Check for duplicate company name (since no unique field specified)
            $checkDuplicate = "SELECT * FROM $table_name WHERE companyname='" . $obj->escape($row_data['companyname']) . "'";
            $chk = $obj->selectdata($table_name, $checkDuplicate);

            if ($chk == "No Data Found!") {
                // Try to insert
                if (insertElectrificationRecord($row_data, $obj, $table_name)) {
                    $inserted_count++;
                } else {
                    $error_count++;
                }
            ...
            <div class='container results-container'>
                <div class='card'>
                    <div class='card-header'>
                        <h4 class='card-title'>Electrification Import Results</h4>
                    </div>
                    <div class='card-body'>
        ...
        echo "<table class='table table-sm summary-table'>
                ...
                <tr><td><strong>Duplicates (Company Name):</strong></td><td><span class='badge bg-warning'>$duplicate_count records</span></td></tr>
                <tr><td><strong>Errors:</strong></td><td><span class='badge bg-danger'>$error_count records</span></td></tr>
              </table>";
        echo "</div>
                        <div class='d-flex gap-2 flex-wrap'>
                            <a href='../editform.php?tablename=electrification' class='btn btn-primary back-btn'>👁️ View Electrification List</a>
                            <a href='../electrification.php' class='btn btn-secondary back-btn'>➕ Add New Provider</a>
                            <a href='../electrification.php' class='btn btn-info back-btn'>⚡ Import More</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <body>
            <div class='container results-container'>
                <div class='alert alert-warning'>
                    ...
                    <p>Please use the downloaded template and ensure you have data in the required fields (Company Name, Office Address, Service Area).</p>
                </div>
                <a href='../electrification.php' class='btn btn-primary'>🔄 Try Again</a>
            </div>
        </body>
        </html>";
    }
    ...
    if (preg_match_all('/<tr[^>]*>(.*?)<\/tr>/s', $html_content, $tr_matches)) {
        foreach ($tr_matches[1] as $index => $tr_content) {
            ...
            if (preg_match_all('/<(?:td|th)[^>]*>(.*?)<\/(?:td|th)>/s', $tr_content, $td_matches)) {
                ...
                // Only add row if we have meaningful data (at least companyname or officeaddress)
...
// Function to insert electrification record into database (villageid auto-added, photo as empty JSON)
function insertElectrificationRecord($data, $obj, $table_name)
{
    try {
        ...
        $field_types = [
            ...
            'energyresourcessolar' => 'boolean',
            'energyresourceswind' => 'boolean',
            'energyresourcescoal' => 'boolean',
            'energyresourcesgas' => 'boolean',
            'photo' => 'json',
            'officeaddress' => 'text',
            'servicearea' => 'text',
            'contactno' => 'string',
            ...
            if (isset($field_types[$field])) {
                switch ($field_types[$field]) {
                    case 'integer':
                        ...
                        break;
                    case 'boolean':
                        // Convert to 1/0 for boolean fields (energy resources)
                        $clean_data[$field] = !empty($value) && (strtolower($value) == '1' || strtolower($value) == 'yes' || strtolower($value) == 'true') ? 1 : 0;
                        break;
                    case 'string':
        ...
        // Build full address like in your form (officeaddress@city@pincode)
        $full_address = $clean_data['officeaddress'];
        if (!empty($clean_data['city'])) {
            $full_address .= '@ ' . $clean_data['city'];
        }
        if (!empty($clean_data['pincode'])) {
            $full_address .= '@ ' . $clean_data['pincode'];
        }
        $clean_data['full_address'] = $full_address;

        // Validate boolean fields (energy resources) - ensure 0 or 1
        $boolean_fields = ['energyresourcessolar', 'energyresourceswind', 'energyresourcescoal', 'energyresourcesgas'];
        foreach ($boolean_fields as $bool_field) {
            if (!in_array($clean_data[$bool_field], [0, 1])) {
                $clean_data[$bool_field] = 0; // Default to false/off
            }
        }

        // Validate email format
        if (!empty($clean_data['email']) && !filter_var($clean_data['email'], FILTER_VALIDATE_EMAIL)) {
            $clean_data['email'] = ''; // Clear invalid email
        }

        // Validate phone numbers (basic 10-digit check)
        $phone_pattern = '/^\d{10}$/';
        if (!empty($clean_data['emergencycontactno']) && !preg_match($phone_pattern, $clean_data['emergencycontactno'])) {
            $clean_data['emergencycontactno'] = '';
        }
        if (!empty($clean_data['contactno']) && !preg_match($phone_pattern, $clean_data['contactno'])) {
            $clean_data['contactno'] = '';
        }

        // Validate pincode (6 digits)
        if (!empty($clean_data['pincode']) && !preg_match('/^\d{6}$/', $clean_data['pincode'])) {
            $clean_data['pincode'] = '';
        }
        ...
        $values = [
            $clean_data['companyname'],
            $clean_data['emergencycontactno'],
            $clean_data['energyresourcessolar'],
            $clean_data['energyresourceswind'],
            $clean_data['energyresourcescoal'],
            $clean_data['energyresourcesgas'],
            $clean_data['photo'], // Empty JSON array
            $clean_data['full_address'], // Combined address@city@pincode
            $clean_data['servicearea'],
            $clean_data['contactno'],
            $clean_data['email'],
            $clean_data['supplychain'],
        ...
        error_log("Electrification import error: " . $e->getMessage());
        return false;
    }
}
