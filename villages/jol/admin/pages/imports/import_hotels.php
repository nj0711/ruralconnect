...
// Define the exact fields from your template (EXCLUDING villageid and hotelsid)
$expected_headers = [
    'hotelname',
    'photo',
    'address',
...
$createTableQuery = "
                       CREATE TABLE `hotels` (
  `hotelsid` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
if (isset($_FILES['excel_file']) && $_FILES['excel_file']['error'] == 0) {
    ...
    if (!empty($imported_data)) {
        ...
        foreach ($imported_data as $row_data) {
            ...
            // Skip if no hotel name (required field based on your schema)
            if (empty($row_data['hotelname'])) {
                $error_count++;
                continue;
            }

            // Check for duplicate hotel name
            $checkDuplicate = "SELECT * FROM $table_name WHERE hotelname='" . $obj->escape($row_data['hotelname']) . "'";
            $chk = $obj->selectdata($table_name, $checkDuplicate);

            if ($chk == "No Data Found!") {
                // Try to insert
                if (insertHotelRecord($row_data, $obj, $table_name)) {
                    $inserted_count++;
                } else {
                    $error_count++;
                }
            ...
            <div class='container results-container'>
                <div class='card'>
                    <div class='card-header'>
                        <h4 class='card-title'>Hotels Import Results</h4>
                    </div>
                    <div class='card-body'>
        ...
        echo "<table class='table table-sm summary-table'>
                ...
                <tr><td><strong>Duplicates (Hotel Name):</strong></td><td><span class='badge bg-warning'>$duplicate_count records</span></td></tr>
                <tr><td><strong>Errors:</strong></td><td><span class='badge bg-danger'>$error_count records</span></td></tr>
              </table>";
        echo "</div>
                        <div class='d-flex gap-2 flex-wrap'>
                            <a href='../editform.php?tablename=hotels' class='btn btn-primary back-btn'>🏨 View Hotels List</a>
                            <a href='../hotels.php' class='btn btn-secondary back-btn'>➕ Add New Hotel</a>
                            <a href='../hotels.php#import-section' class='btn btn-info back-btn'>📥 Import More</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <body>
            <div class='container results-container'>
                <div class='alert alert-warning'>
                    ...
                    <p>Please use the downloaded template and ensure you have data in the required fields (Hotel Name).</p>
                </div>
                <a href='../hotels.php' class='btn btn-primary'>🔄 Try Again</a>
            </div>
        </body>
        </html>";
    }
    ...
    if (preg_match_all('/<tr[^>]*>(.*?)<\/tr>/s', $html_content, $tr_matches)) {
        foreach ($tr_matches[1] as $index => $tr_content) {
            ...
            if (preg_match_all('/<(?:td|th)[^>]*>(.*?)<\/(?:td|th)>/s', $tr_content, $td_matches)) {
                ...
                // Only add row if we have meaningful data (at least hotelname)
...
// Function to insert hotel record into database (villageid auto-added, photo as empty JSON)
function insertHotelRecord($data, $obj, $table_name)
{
    try {
        ...
        error_log("Hotels import error: " . $e->getMessage());
        return false;
    }
}
