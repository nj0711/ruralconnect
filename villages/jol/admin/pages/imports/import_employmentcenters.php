<?php
// imports/import_employmentcenters.php
...
// Define the exact fields from your template (EXCLUDING villageid and employmentcentersid)
$expected_headers = [
    'centername',
    'servicetype',
    'contactnumber',
...
$createTableQuery = "
  ...
  `employmentcentersid` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
if (!$Emp->tableExists('employmentcenters')) {
    if (!$Emp->mysqli->query($createTableQuery)) {
        echo "<script>alert('Error creating table: " . $Emp->mysqli->error . "');</script>";
    }
}

$table_name = 'employmentcenters';
...
if (isset($_FILES['excel_file']) && $_FILES['excel_file']['error'] == 0) {
    ...
    if (!empty($imported_data)) {
        ...
        $village_res = $Emp->selectdata("villagebasic", "select villageid from villagebasic");
        ...
        foreach ($imported_data as $row_data) {
            ...
            // Skip if no center name, service type, or contact number (required fields based on your schema)
            if (empty($row_data['centername']) || empty($row_data['servicetype']) || empty($row_data['contactnumber'])) {
                $error_count++;
                continue;
            }

            // Check for duplicate center name + service type combination
            $checkDuplicate = "SELECT * FROM $table_name WHERE centername='" . $Emp->escape($row_data['centername']) . "' AND servicetype='" . $Emp->escape($row_data['servicetype']) . "'";
            $chk = $Emp->selectdata($table_name, $checkDuplicate);

            if ($chk == "No Data Found!") {
                // Try to insert
                if (insertEmploymentCenterRecord($row_data, $Emp, $table_name)) {
                    $inserted_count++;
                } else {
                    $error_count++;
                }
            ...
            <div class='container results-container'>
                <div class='card'>
                    <div class='card-header'>
                        <h4 class='card-title'>Employment Centers Import Results</h4>
                    </div>
                    <div class='card-body'>
        ...
        echo "<table class='table table-sm summary-table'>
                ...
                <tr><td><strong>Duplicates (Center+Type):</strong></td><td><span class='badge bg-warning'>$duplicate_count records</span></td></tr>
                <tr><td><strong>Errors:</strong></td><td><span class='badge bg-danger'>$error_count records</span></td></tr>
              </table>";
        echo "</div>
                        <div class='d-flex gap-2 flex-wrap'>
                            <a href='../editform.php?tablename=employmentcenters' class='btn btn-primary back-btn'>💼 View Employment Centers</a>
                            <a href='../employmentcenters.php' class='btn btn-secondary back-btn'>➕ Add New Center</a>
                            <a href='../employmentcenters.php#import-section' class='btn btn-info back-btn'>📥 Import More</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <body>
            <div class='container results-container'>
                <div class='alert alert-warning'>
                    ...
                    <p>Please use the downloaded template and ensure you have data in the required fields (Center Name, Service Type, Contact Number).</p>
                </div>
                <a href='../employmentcenters.php' class='btn btn-primary'>🔄 Try Again</a>
            </div>
        </body>
        </html>";
    }
    ...
    if (preg_match_all('/<tr[^>]*>(.*?)<\/tr>/s', $html_content, $tr_matches)) {
        foreach ($tr_matches[1] as $index => $tr_content) {
            ...
            if (preg_match_all('/<(?:td|th)[^>]*>(.*?)<\/(?:td|th)>/s', $tr_content, $td_matches)) {
                ...
                // Only add row if we have meaningful data (at least centername, servicetype, or contactnumber)
...
// Function to insert employment center record into database (villageid auto-added)
function insertEmploymentCenterRecord($data, $Emp, $table_name)
{
    try {
            ...
            if (isset($field_types[$field])) {
                switch ($field_types[$field]) {
                    case 'integer':
                        ...
                        break;
                    case 'string':
                        $clean_data[$field] = !empty($value) ? $Emp->escape(trim($value)) : '';
                        break;
                    case 'text':
                        $clean_data[$field] = !empty($value) ? $Emp->escape(trim($value)) : '';
                        break;
        ...
        // Validate service type (Job Placement, Skill Training, Career Counseling, Other)
        $valid_types = ['Job Placement', 'Skill Training', 'Career Counseling', 'Other'];
        ...
        $result = $Emp->insertdata($table_name, $query);

        return ($result == "Data Inserted.");
    } catch (Exception $e) {
        error_log("Employment centers import error: " . $e->getMessage());
        return false;
    }
}
