<?php
// imports/import_tourismplaces.php
...
// Define the exact fields from your template (EXCLUDING tourismplacesid and villageid)
$expected_headers = [
    'name',
    'type',
    'address',
...
$createTableQuery = "
  ...
  `tourismplacesid` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
if (isset($_FILES['excel_file']) && $_FILES['excel_file']['error'] == 0) {
    ...
    if (!empty($imported_data)) {
        ...
        foreach ($imported_data as $row_data) {
            ...
            $full_address = trim(($row_data['address'] ?? '') . '@' . ($row_data['city'] ?? '') . '@' . ($row_data['zip'] ?? ''));
            $row_data['address'] = $full_address;

            // Skip if no name or required fields (based on your form validation)
            if (
                empty($row_data['name']) || empty($row_data['address']) ||
                empty($row_data['contactno']) || empty($row_data['city']) ||
                empty($row_data['zip']) || empty($row_data['amenitiesfacilities']) ||
                empty($row_data['entryfees']) || empty($row_data['history'])
            ) {
                $error_count++;
                continue;
            }

            // Check for duplicate name (same logic as your form - adjust if you have a unique field)
            $checkName = "SELECT * FROM $table_name WHERE name='" . $obj->escape($row_data['name']) . "'";
            $chk = $obj->selectdata($table_name, $checkName);

            if ($chk == "No Data Found!") {
                // Try to insert
                if (insertTourismPlaceRecord($row_data, $obj, $table_name)) {
                    $inserted_count++;
                } else {
                    $error_count++;
                }
            ...
            <div class='container results-container'>
                <div class='card'>
                    <div class='card-header'>
                        <h4 class='card-title'>Import Results - Tourism Places</h4>
                    </div>
                    <div class='card-body'>
        ...
        echo "</div>
                        <div class='d-flex gap-2'>
                            <a href='../editform.php?tablename=tourismplaces' class='btn btn-primary back-btn'>👁️ View Tourism Places List</a>
                            <a href='../tourismplaces.php' class='btn btn-secondary back-btn'>➕ Add New Tourism Place</a>
                            <a href='../tourismplaces.php' class='btn btn-info back-btn'>📥 Import More</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <body>
            <div class='container results-container'>
                <div class='alert alert-warning'>
                    ...
                    <p>Please use the downloaded template and ensure you have data in the required fields (Name, Address, Contact No, etc.).</p>
                </div>
                <a href='../tourismplaces.php' class='btn btn-primary'>🔄 Try Again</a>
            </div>
        </body>
        </html>";
    }
...
// Function to parse HTML table format (matches your template - 16 columns)
    ...
    if (preg_match_all('/<tr[^>]*>(.*?)<\/tr>/s', $html_content, $tr_matches)) {
        foreach ($tr_matches[1] as $index => $tr_content) {
            ...
            if (preg_match_all('/<(?:td|th)[^>]*>(.*?)<\/(?:td|th)>/s', $tr_content, $td_matches)) {
                ...
                // Only add row if we have meaningful data (at least name or contactno)
...
// Function to parse actual XLSX file (16 columns)
...
// Function to insert record into database (villageid and tourismplacesid auto-handled)
function insertTourismPlaceRecord($data, $obj, $table_name)
{
    try {
            ...
            if (isset($field_types[$field])) {
                switch ($field_types[$field]) {
                    ...
                    case 'text':
                        ...
                        break;
                    case 'json':
                        // Handle timeduration as JSON (same structure as your form)
                        if ($field == 'timeduration') {
                            $time_json = json_encode([
                                'weekdayopenkey' => $data['weekdayopenkey'] ?? '',
                                'weekdayclosekey' => $data['weekdayclosekey'] ?? '',
                                'weekendopenkey' => $data['weekendopenkey'] ?? '',
                                'weekendclosekey' => $data['weekendclosekey'] ?? ''
                            ]);
        ...
        // Build the INSERT query (matches your existing table structure)
        $columns = [
            'villageid',
            'name',
            'type',
        ...
        $values = [
            ...
            $clean_data['type'],
            $clean_data['address'],
            $clean_data['timeduration'],
            $clean_data['contactno'],
            $clean_data['email'],
            $clean_data['amenitiesfacilities'],
            $clean_data['entryfees'],
            $clean_data['history'],
            $clean_data['photo'],
        ...
        error_log("Tourism places import error: " . $e->getMessage());
        return false;
    }
}
