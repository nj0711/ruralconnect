<?php
// imports/import_fuelstation.php
...
// Define the exact fields from your template (EXCLUDING villageid and fuelstationid)
$expected_headers = [
    'fuelstationname',
    'photo',
    'address',
...
$createTableQuery = "
  ...
  `fuelstationid` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
if (isset($_FILES['excel_file']) && $_FILES['excel_file']['error'] == 0) {
    ...
    if (!empty($imported_data)) {
        ...
        foreach ($imported_data as $row_data) {
            ...
            // Skip if no station name or fuel types (required fields based on your schema)
            if (empty($row_data['fuelstationname']) || empty($row_data['typesoffuelavailable'])) {
                $error_count++;
                continue;
            }

            // Check for duplicate station name
            $checkDuplicate = "SELECT * FROM $table_name WHERE fuelstationname='" . $obj->escape($row_data['fuelstationname']) . "'";
            $chk = $obj->selectdata($table_name, $checkDuplicate);

            if ($chk == "No Data Found!") {
                // Try to insert
                if (insertFuelStationRecord($row_data, $obj, $table_name)) {
                    $inserted_count++;
                } else {
                    $error_count++;
                }
            ...
            <div class='container results-container'>
                <div class='card'>
                    <div class='card-header'>
                        <h4 class='card-title'>Fuel Station Import Results</h4>
                    </div>
                    <div class='card-body'>
        ...
        echo "</div>
                        <div class='d-flex gap-2 flex-wrap'>
                            <a href='../editform.php?tablename=fuelstation' class='btn btn-primary back-btn'>⛽ View Fuel Stations</a>
                            <a href='../fuelstation.php' class='btn btn-secondary back-btn'>➕ Add New Station</a>
                            <a href='../fuelstation.php' class='btn btn-info back-btn'>📥 Import More</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <body>
            <div class='container results-container'>
                <div class='alert alert-warning'>
                    ...
                    <p>Please use the downloaded template and ensure you have data in the required fields (Station Name, Fuel Types).</p>
                </div>
                <a href='../fuelstation.php' class='btn btn-primary'>🔄 Try Again</a>
            </div>
        </body>
        </html>";
    }
...
// Function to parse HTML table format (matches your template - 9 columns now)
    ...
    if (preg_match_all('/<tr[^>]*>(.*?)<\/tr>/s', $html_content, $tr_matches)) {
        foreach ($tr_matches[1] as $index => $tr_content) {
            ...
            if (preg_match_all('/<(?:td|th)[^>]*>(.*?)<\/(?:td|th)>/s', $tr_content, $td_matches)) {
                ...
                // Only add row if we have meaningful data (at least fuelstationname or typesoffuelavailable)
...
// Function to parse actual XLSX file (9 columns now)
...
// Function to insert fuel station record into database (villageid auto-added, photo as empty JSON)
function insertFuelStationRecord($data, $obj, $table_name)
{
    try {
        ...
        $field_types = [
            ...
            'zip' => 'string',
            'contactno' => 'string',
            'pumpstimeduration' => 'json',
            'typesoffuelavailable' => 'string',
            'description' => 'text',
            'photo' => 'json',
            'visibility' => 'string'
        ];
            ...
            if (isset($field_types[$field])) {
                switch ($field_types[$field]) {
                    ...
                    case 'text':
                        ...
                        break;
                    case 'json':
                        // Handle pumpstimeduration as JSON (open/close times)
                        if ($field == 'pumpstimeduration') {
                            // Try to parse JSON from Excel, or create empty structure
                            $time_json = !empty($value) ? $value : json_encode([
                                'open' => '',
                                'close' => ''
                            ]);

                            // Validate JSON format
                            $decoded = json_decode($time_json, true);
                            if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                                $clean_data[$field] = $time_json;
                            } else {
                                // Create default structure if invalid JSON
        ...
        // Validate fuel types (comma-separated string)
        $valid_fuel_types = ['petrol', 'diesel', 'cng', 'other'];
        if (!empty($clean_data['typesoffuelavailable'])) {
            $fuel_array = explode(',', $clean_data['typesoffuelavailable']);
            $fuel_array = array_map('trim', $fuel_array);
            $valid_fuel_array = array_intersect($fuel_array, $valid_fuel_types);
            $clean_data['typesoffuelavailable'] = implode(',', $valid_fuel_array);

            if (empty($clean_data['typesoffuelavailable'])) {
                $clean_data['typesoffuelavailable'] = 'petrol'; // Default to petrol
            }
        } else {
            $clean_data['typesoffuelavailable'] = 'petrol'; // Default if empty
        }

        // Validate contact number (10-digit phone)
        if (!empty($clean_data['contactno']) && !preg_match('/^\d{10}$/', $clean_data['contactno'])) {
            $clean_data['contactno'] = ''; // Clear invalid phone
        }
        ...
        $values = [
            ...
            $clean_data['fuelstationname'],
            $clean_data['full_address'], // Combined address@city@zip
            $clean_data['contactno'],
            $clean_data['pumpstimeduration'],
            $clean_data['typesoffuelavailable'],
        ...
        error_log("Fuel station import error: " . $e->getMessage());
        return false;
    }
}
